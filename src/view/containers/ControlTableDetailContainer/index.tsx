...
import DataLineChart from '../../components/DataLineChart/index';
import InformationTable from '../../containers/InformationTable/index';
...
import TextEllipsis from '../../components/ColumnRenderers/TextEllipsis/index';
...
export default function ControlTableDetailContainer({ uuid }: any) {
    const { t, i18n } = useTranslation();
    const classes: any = useStyles();
    const setCurrentMenu = useSetRecoilState<any>(currentMenuState);
    const [graphConfig, setGraphConfig] = useState<any>({
        data: [],
        annotations: [],
    });
    const [inspectAndAlarm, setInspectAndAlarm] = useState<any>(null);
    const [tableDataInTransport, setTableDataInTransport] = useState<any>({
        result: [],
    });
    ...
    const [graphMinMax, setGraphMinMax] = useState<any>(null);
    const [alarmData, setAlarmData] = useState<any>({ result: [] });
    ...
    useEffect(() => {
        if (controlDetailData) {
            const { transportHistoryInProduct }: any = controlDetailData;
    ...
    useEffect(() => {
        if (menuConfig) {
            ...
            setGraphConfig({
                data: temperature.map((value: any, idx: any) => ({
    ...
    useEffect(() => {
        if (menuConfig) {
            if (graphConfig.data !== 0) {
                setAlarmData({
                    result: graphConfig.data.filter((r: any) =>
