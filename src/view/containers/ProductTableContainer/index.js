...
const defaultColumns = [
    ...
    },
    {
        ...
        width: '150px',
        dataIndex: 'modifyKey',
        render: (key) => <ProductChangeButton id={key} />,
    },
];
...
const moreColumnsByAdminM = [
    {
        ...
        width: '150px',
        dataIndex: 'deleteKey',
        render: (key) => <ProductDeleteButton id={key} />,
    },
];

// 변경 버튼
function ProductChangeButton({ id: productId }) {
    const { t } = useTranslation();
    const setModal = useSetRecoilState(productModalState);
...
// 삭제 버튼
function ProductDeleteButton({ id: productId }) {
    const { t } = useTranslation();
...
export default function ProductTableContainer() {
    ...
    useEffect(() => {
        if (productsData) {
            ...
            const { result, pages } = tableMapper(productsByCompany, i18n.language);
