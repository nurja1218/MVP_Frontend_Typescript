...
const defaultColumns = [
    ...
    },
    {
        ...
        width: '10%',
        dataIndex: 'roleId',
        render: (state) => <RoleBadge status={state} />,
...
// 변경 버튼
function ProductChangeButton({ id: accountId, role, email }) {
    const { email: clientEmail } = useRecoilValue(userState);
...
function AccountDeleteButton({ id: accountId, role }) {
    const { t } = useTranslation();
    // 삭제 버튼 효과
    const [hoverd, setHoverd] = useState(false);
    const handleMouseOver = () => {
...
export default function AccountTableContainer() {
    ...
    const [searchType, setSearchType] = useState(defaultOptionValue);
    const {
        data: usersData,
        error: usersError,
        refetch,
    ...
    const handleExportXLSX = () => {
        exportToXLSX({
            data: tableDataMapper(defaultColumns, tableData.result, {
                roleId: (value) => ACCOUNT_ROLES[value],
            }),
            configs: {
                title: '사용자관리',
            },
