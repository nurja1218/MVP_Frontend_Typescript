...
// 모달 내 추가, 삭제 버튼
function SerialNoAddButton({ onClick }) {
...
function SerialNoDeleteButton({ onClick }) {
    const classes = useStyles();
    return (
        <span className={classes.serialNoButton} onClick={onClick}>
...
export default function ProductModal() {
    ...
    const [modal, setModal] = useRecoilState(productModalState);
    const [product, setProduct] = useState(null);
    const [dataRange, setDataRange] = useState(defaultDataRange);
    const [isRangeAbnormal, setIsRangeAbnormal] = useState(defaultIsRangeAbnormal);
    const classes = useStyles();
    ...
    const [serialRangeList, setSerialRangeList] = useState([
        {
            key: 0,
        },
    ]);
    ...
    const handleRemoveClick = (key) => {
        const newList = serialList.filter((item) => item.key !== key);
        setSerialList(newList);
    };
    const handleRangeRemoveClick = (key) => {
        const newList = serialRangeList.filter((item) => item.key !== key);
                                            ...
                                            return (
                                                ...
                                                >
                                                    <Grid item xs={12} sm={4}>
                                                        <Form.Item name={`serial-${item.key}`}>
                                                            <InputNumber
