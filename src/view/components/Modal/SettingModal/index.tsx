...
export default function SettingModal({ status }: any) {
    const [form] = Form.useForm();
    const { roleId: clientType } = useRecoilValue(userState);
    const TabPane = Tabs.TabPane;
    const { t } = useTranslation();
    const [tabStatus, setTabStatus] = useState<any>('userInfo');
    const [modifyAccountSelf] = useMutation(MODIFY_USER_AND_COMPANY);
    const [changePassword] = useMutation(CHANGE_PASSWORD);
    const [currentUser, setCurrentUser] = useState<any>(null);
    const [modal, setModal] = useRecoilState<any>(settingModalState);
    const logout: any = useSetRecoilState(logoutHandler);
