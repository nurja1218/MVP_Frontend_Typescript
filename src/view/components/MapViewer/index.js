...
const generatePolylines = (markers) => {
    if (markers.length <= 1) {
        return [];
    }
    const polylines = [];
...
function MapWithGoogleMap(transportMarkers, zoom) {
    ...
    useEffect(() => {
        if (transportMarkers.length > 0) {
            setMarkers(transportMarkers.filter((mark) => mark !== false));
        }
    }, [transportMarkers]);
...
export default memo(function MapViewer({ zoom = 11, transportMarkers, height = '100%' }) {
